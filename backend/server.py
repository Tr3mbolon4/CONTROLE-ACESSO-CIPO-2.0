...
async def login(data: UserLogin, request: Request, response: Response):
    ...
    if attempts and attempts.get("count", 0) >= 5:
        ...
        # Ensure timezone-awareness (MongoDB may return naive datetime)
...
# When credentials are enabled, wildcard is not allowed by browsers.
# Fall back to permissive regex to keep preview/production flexible.
if _allowed_origins == ["*"]:
    app.add_middleware(
        CORSMiddleware,
        allow_origin_regex=".*",
